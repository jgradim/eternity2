...
Solving Eternity II with \textit{GA}'s implies that we must first choose the state that will represent our known solutions in the search space. Our state, or in the context of \textit{GA}'s', \texit{individiual} or \textit{chromosome} is the trivial solution to this problem: the board itself.
