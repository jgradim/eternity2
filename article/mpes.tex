...
%Solving Eternity II with \textit{GA}s implies that we must first choose the state that will represent our known solutions in the search space. Our state, or in the context of \textit{GA}'s, \textit{individiual} or \textit{chromosome} is the trivial solution to this problem: the board itself.
...
%TODO: rever algoritmo, tornar mais legivel, etc
\begin{algorithm}
...
\newcommand{\logicaland}{\textbf{and}\ }
	...
	\FOR{$i \gets 1$ to $n_{squares}$}
		...
		\ENDIF\label{sec:implementation}
	\ENDFOR
	\RETURN $features$
	\end{algorithmic}
...
\begin{figure}[h]
	\centering
	%\includegraphics[width=35mm]{images/population_stats.png}
